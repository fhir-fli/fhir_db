...
import 'package:fhir_db/src/has_parameter.dart';
...
import 'package:fhir_db/src/search/search_escaping.dart';
...
class FhirDao<R extends FhirNode, T extends Object>
    extends DatabaseAccessor<FhirDb<R, T>> {
  ...
  @visibleForTesting
  late SearchParameterLists Function(R resource) extractSearchParameters =
      (resource) => extractWithContained(model, resource);
  ...
  Future<R> saveResource(
    ...
    final newResource = await transaction(() async {
      final existingRow =
          await _currentRow(model.typeFromName(withId.fhirType)!, id);
  ...
  Future<bool> saveResources(
    ...
    try {
      ...
      await transaction(() async {
        final withIds = [for (final r in resourcesList) _withIdIfNone(r)];
        ...
        for (final resource in withIds) {
          ...
          if (previous != null) {
            ...
            final earlier = currentRows.lastWhere(
              (c) =>
                  c.resourceType.value == resource.fhirType &&
                  c.id.value == resource.resourceId,
  ...
  /// The `meta` of the next version, as JSON: `versionId` counted from
  /// [stored] (or a timestamp under [versionIdAsTime]), `lastUpdated` now,
  /// and, with [mergeTags], the stored `tag` and `security` codings the
  /// submitted ones do not repeat (system+code) kept in front of them.
  /// Everything else of [submitted] (profile, source, extensions) stands.
  Map<String, dynamic> _nextMeta(
    ...
    if (versionIdAsTime) {
      versionId = now.toIso8601String().replaceAll(':', '.');
    } else {
      final counted =
          storedVersion == null ? null : int.tryParse(storedVersion);
      versionId = '${(counted ?? 0) + 1}';
    }
    ...
    if (mergeTags && stored != null) {
      for (final field in ['tag', 'security']) {
        final merged = _mergeCodings(
          (stored[field] as List?)?.cast<Map<String, dynamic>>(),
          (submitted?[field] as List?)?.cast<Map<String, dynamic>>(),
        );
        if (merged == null) {
          meta.remove(field);
        } else {
  ...
  List<Map<String, dynamic>>? _mergeCodings(
    List<Map<String, dynamic>>? stored,
    List<Map<String, dynamic>>? submitted,
  ) {
    if (stored == null || stored.isEmpty) return submitted;
    String key(Map<String, dynamic> c) => '${c['system']}|${c['code']}';
    final submittedKeys = {
      for (final c in submitted ?? const <Map<String, dynamic>>[]) key(c),
    };
    return [
      for (final c in stored)
        if (!submittedKeys.contains(key(c))) c,
      ...?submitted,
    ];
  }

  /// The `meta` of [resource] as JSON, or null when it has none.
  Map<String, dynamic>? _metaJsonOf(R resource) {
    final meta = resource.child('meta');
    return meta == null ? null : model.jsonOf(meta);
  }

  /// The `meta` inside a stored resource's JSON text, or null.
  Map<String, dynamic>? _metaJsonOfText(String json) {
  ...
  /// [resource] as it is, or with a new id when it has none.
  R _withIdIfNone(R resource) => resource.resourceId == null
      ? model.withId(resource, newResourceId())
      : resource;
  ...
  Future<
      Map<
          ...
          ({
            ...
            ResourcesHistoryCompanion history
          })>> _storedRows(List<R> resourcesList) async {
    final rows = <String,
        ({Map<String, dynamic>? meta, ResourcesHistoryCompanion history})>{};
    final idsByType = <String, Set<String>>{};
    for (final r in resourcesList) {
      idsByType.putIfAbsent(r.fhirType, () => {}).add(r.resourceId!);
    }
    const chunk = 500;
  ...
  Future<List<R>> getResourcesByType(
    T resourceType,
  ) async {
    final resourceTypeString = resourceType.toString();
    final query = select(resources)
  ...
  Future<List<R>> getResourcesWithPagination({
  ...
  /// How many resources of [resourceType] the store holds.
  Future<int> getResourceCount(T resourceType) =>
      countResourcesOfType(resourceType.toString());

  /// [getResourceCount] by type name.
  Future<int> countResourcesOfType(String resourceType) async {
    final resourceTypeString = resourceType;
  ...
  Future<List<T>> getResourceTypes() async {
    final results = await customSelect(
      'SELECT DISTINCT resource_type FROM resources;',
      readsFrom: {resources},
    ).get();

    final resourceTypes = <T>[];
    for (final row in results) {
      final resourceTypeString = row.data['resource_type'] as String;
      final resourceType = model.typeFromName(resourceTypeString);
  ...
  /// not be read at all.
  Future<List<R>> getResourceHistory(
    T resourceType,
    String id,
  ) async =>
      [
  ...
  Future<List<HistoryEntry<R>>> getHistory(
    T resourceType,
    String id, {
    DateTime? since,
    DateTime? at,
    ...
    return [
      for (final row in rows)
        HistoryEntry<R>.fromRow(resourcesHistory.map(row.data), model),
    ];
  }
  ...
  Future<HistoryEntry<R>?> getVersion(
    T resourceType,
    String id,
    String versionId,
  ) async {
    ...
    final row = await (select(resourcesHistory)
        ...
        .getSingleOrNull();
    return row == null ? null : HistoryEntry<R>.fromRow(row, model);
  }

  /// Check if a resource exists.
  Future<bool> exists(T resourceType, String id) async {
    final resourceTypeString = resourceType.toString();
    final query = select(resources)
      ..where(
        (tbl) =>
  ...
  Future<List<R>> _hydrate(
    T resourceType,
    List<String> ids,
  ) async {
    if (ids.isEmpty) return [];
    final byId = <String, R>{
      for (final r in await getResources(resourceType, ids)) r.resourceId!: r,
    };
    return [
      for (final id in ids)
  ...
  Future<List<String>?> _pagedIds(
    ...
    if (sortKeys.isEmpty && count != null && parts.isNotEmpty) {
      ...
      if (sized.length > 1) {
        ...
        // single part.
        final total = await countResourcesOfType(resourceType);
        if (total > 0) {
          var selectivity = 1.0;
  ...
  Expression<bool>? _compartmentParamCondition(
    ...
    final params = model.compartmentDefinitions[scope.type]?[resourceType];
  ...
  Future<_IndexCondition?> _compartmentCondition(
    ...
    if (!model.compartmentDefinitions.containsKey(scope.type)) return null;
  ...
  Future<void> _saveToSync(R resource) async {
    await into(syncResources).insertOnConflictUpdate(
      SyncResourcesCompanion(
        ...
        versionId: Value(resource.metaVersionId!),
      ),
    );
  }
  ...
  /// Retrieve all canonical resources of a type. [C] narrows the result
  /// to a class of the binding's model (`CanonicalResource`).
  Future<List<C>> getAllCanonicalByType<C extends R>(T type) async {
    final query = select(canonicalResources)
  ...
  Future<void> saveCanonicalResource(R resource) async {
  ...
  Future<R?> getCanonicalResource(String url) async {
    final query = select(canonicalResources)
  ...
  Future<void> _updateSearchParametersBulk(
  ...
  ) async {
    {
      ...
      final last = <String, R>{
        for (final r in resourcesList) '${r.fhirType}/${r.resourceId!}': r,
      };
  ...
  Future<List<ExpandedCode>> expandValueSet(R valueSet) async {
    ...
    if (contains != null && contains.isNotEmpty) {
      _extractFromContains(contains, codes);
      return codes;
    }
...
class HistoryEntry<R extends FhirNode> {
  ...
  factory HistoryEntry.fromRow(
    ResourcesHistoryData row,
    FhirModel<R, Object> model,
  ) {
